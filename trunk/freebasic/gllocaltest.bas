...
sub onCallback cdecl(byval webView as any ptr, byval objectName as wstring ptr, byval callbackName as wstring ptr, byval args as any ptr)
	info "callback"
	info str(awe_JSValue_toInteger(awe_JSArguments_get(args, 0)))
	info str(awe_JSValue_toInteger(awe_JSArguments_get(args, 1)))
end sub
...
sub init(url as string)
	...
	awe_WebView_loadFile(webView, "data/web/html/debug.html", "")	
	awe_WebView_focus(webView)	
	
	'' test callbacks
	awe_WebView_createObject(webView, "derp")
	awe_WebView_setObjectCallback(webView, "derp", "mycallback")
